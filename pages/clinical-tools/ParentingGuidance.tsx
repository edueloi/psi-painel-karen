...
export const ParentingGuidancePage: React.FC = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      if (!patientId) return;
      try {
        const response = await fetch(`/api/clinical-tools/${patientId}/parenting-guidance`, {
  ...
  return (
    <div className="space-y-8 animate-fadeIn">
      <PageHeader
        ...
        subtitle="Construção de rotinas visuais e manejo de contingências comportamentais."
        ...
        <div className="lg:col-span-2 space-y-8">
           <div className="bg-white rounded-[2.5rem] border border-slate-100 p-10 shadow-sm space-y-10">
              <div className="flex items-center justify-between">
                 ...
                 <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest bg-slate-50 px-3 py-1.5 rounded-xl border border-slate-100 flex items-center gap-2">
              ...
              <div className="space-y-4">
                 {data.routine.sort((a,b) => a.time.localeCompare(b.time)).map(item => (
                    <div key={item.id} className="group flex items-center justify-between p-6 bg-slate-50 rounded-3xl border border-slate-100 transition-all hover:bg-white hover:border-amber-200">
                       <div className="flex items-center gap-8">
                          <div className="w-20 h-20 bg-amber-50 rounded-2xl flex flex-col items-center justify-center border border-amber-100">
                 ...
                 <div className="bg-slate-50/50 p-8 rounded-[2rem] border border-slate-100 border-dashed grid grid-cols-1 md:grid-cols-12 gap-6 items-end mt-10">
                    <div className="md:col-span-2 space-y-2">
                       ...
                       <input 
                          ...
                          className="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-xs font-bold outline-none ring-4 ring-transparent focus:ring-amber-50 transition-all font-mono"
                       />
                    </div>
                    <div className="md:col-span-7 space-y-2">
                       <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Atividade/Momento</label>
                       <input 
                          placeholder="Ex: Hora do banho, Jantar em família..."
                          value={newRoutine.activity}
                          onChange={e => setNewRoutine({...newRoutine, activity: e.target.value})}
                          className="w-full bg-white border border-slate-200 rounded-xl px-5 py-3 text-xs font-bold outline-none ring-4 ring-transparent focus:ring-amber-50 transition-all"
                       />
                    </div>
              ...
              <div className="space-y-6 relative z-10">
                 <div className="space-y-2">
                    ...
                    <textarea 
                       ...
                       className="w-full bg-white/5 border border-white/10 rounded-3xl p-6 text-xs font-bold leading-relaxed focus:ring-4 focus:ring-amber-500/20 outline-none transition-all placeholder:text-white/10 resize-none italic"
                       placeholder="Quais são as principais mudanças esperadas no comportamento dos pais e da criança?"
                    />
                 </div>

                 <div className="p-8 bg-amber-500/10 rounded-3xl border border-amber-500/20 space-y-4">
                    <h4 className="text-[10px] font-black text-amber-500 uppercase tracking-widest flex items-center gap-2">
                       <AlertCircle size={14} /> Nota Técnica do Psi
                    </h4>
                    <p className="text-[11px] font-bold text-slate-300 leading-relaxed">
                       O foco deste painel é a **Psicoeducação**. Utilize o registro de rotina para validar o que os pais conseguem implementar na semana.
                    </p>
                 </div>
