...
export const CoupleTherapyPage: React.FC = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      if (!patientId) return;
      try {
        const response = await fetch(`/api/clinical-tools/${patientId}/couple-therapy`, {
  ...
  return (
    <div className="space-y-8 animate-fadeIn">
      <PageHeader
        ...
        subtitle="Mapeamento de vínculos, contratos e mapas do amor."
              ...
              <div className="bg-slate-50/50 p-6 rounded-[2rem] border border-slate-100/50 border-dashed space-y-5">
                 <div className="flex gap-4">
                    <select 
                       ...
                       className="bg-white border border-slate-200 rounded-xl px-4 py-2 text-[10px] font-black uppercase outline-none focus:ring-4 focus:ring-emerald-50"
                    >
                       <option value="A">Parceiro A</option>
                       <option value="B">Parceiro B</option>
                    </select>
                    <input 
                       placeholder="Novo sonho, interesse ou medo registrado..."
                       value={newMapItem.interest}
                       onChange={e => setNewMapItem({...newMapItem, interest: e.target.value})}
                       className="flex-1 bg-white border border-slate-200 rounded-xl px-5 py-3 text-xs font-bold outline-none focus:ring-4 focus:ring-emerald-50"
                    />
                    <Button 
                       onClick={addMapItem}
                       className="bg-emerald-600 hover:bg-slate-950 text-white rounded-xl px-6 py-3 font-black uppercase text-[10px] tracking-widest"
                    >
                       Inserir <Plus size={16} className="ml-2" />
                    </Button>
                 </div>
              </div>
           </div>
        ...
        <div className="lg:col-span-2 space-y-8">
           ...
           <div className="bg-slate-950 rounded-[2.5rem] p-10 text-white shadow-2xl space-y-8 relative overflow-hidden">
              ...
              <div className="space-y-4 relative z-10">
                 <textarea 
                    rows={8}
                    className="w-full bg-white/5 border border-white/10 rounded-[2rem] p-8 text-xs font-bold leading-relaxed focus:ring-4 focus:ring-emerald-500/20 outline-none transition-all placeholder:text-white/20 resize-none"
