...
import { Comanda, ComandaStatus, ComandaType, Frequency, ServicePackage } from '../types';
import { 
  ShoppingBag, Search, Plus, Filter, Edit3, Trash2, Eye, Calendar, User, 
...
export const Comandas: React.FC = () => {
  ...
  const filteredComandas = comandas.filter(c => {
      ...
      if (activeTab === 'fechada') {
          // Simple date filter logic for demo (checking created date or end date)
  ...
  const handleSaveComanda = () => {
      if (!editingComanda?.patientId || !editingComanda.description) {
          alert('Preencha o paciente e a descrição.');
          return;
      }
  ...
  const handleSelectPackage = (packageId: string) => {
      ...
      // Convert Package Items to Comanda Items
      ...
      // Apply package logic (override discount handling for simplicity or keep package price)
      // Here we set the discount to match the package total price logic if needed, 
      // OR just add items and let the user set the package price manually via discount.
      // For better UX, let's pre-set the discount based on the package definition.
      ...
      <div className="relative overflow-hidden rounded-[26px] p-8 bg-slate-900 shadow-2xl shadow-emerald-900/20 border border-slate-800">
        ...
        <div className="relative z-10 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-8">
            <div className="max-w-2xl">
                ...
                <h1 className="text-3xl md:text-4xl font-display font-bold text-white mb-3 leading-tight">Comandas & Pedidos</h1>
                <p className="text-emerald-100/70 text-lg leading-relaxed max-w-xl">
                    Gerencie vendas, pacotes e controle os recebimentos de cada paciente de forma organizada.
                </p>
            </div>
            ...
            <div className="flex bg-slate-800/50 p-1 rounded-2xl border border-slate-700/50 backdrop-blur-sm">
                <button 
                    ...
                    className={`px-6 py-3 rounded-xl font-bold transition-all flex items-center gap-2 ${activeTab === 'aberta' ? 'bg-emerald-600 text-white shadow-lg' : 'text-slate-400 hover:text-white'}`}
                >
                    <Clock size={18} /> Abertas
                </button>
                <button 
                    onClick={() => setActiveTab('fechada')}
                    className={`px-6 py-3 rounded-xl font-bold transition-all flex items-center gap-2 ${activeTab === 'fechada' ? 'bg-emerald-600 text-white shadow-lg' : 'text-slate-400 hover:text-white'}`}
                >
                    <Archive size={18} /> Fechadas
                </button>
            </div>
        </div>
      </div>
            ...
            <div className="flex flex-1 w-full gap-4">
                <div className="relative flex-1 max-w-md group">
                    ...
                    <input 
                        ...
                        className="w-full pl-12 pr-4 py-3.5 bg-white border border-slate-200 rounded-2xl focus:outline-none focus:ring-2 focus:ring-emerald-100 focus:border-emerald-300 transition-all text-slate-600 shadow-sm"
                    />
                </div>
                <button 
                    onClick={() => handleOpenModal()}
                    className="bg-emerald-600 hover:bg-emerald-700 text-white px-6 py-3.5 rounded-2xl font-bold shadow-lg shadow-emerald-200 transition-all flex items-center gap-2 whitespace-nowrap"
                >
                    <Plus size={20} /> <span className="hidden md:inline">Nova Comanda</span>
                </button>
            </div>

            {/* Filters for Closed Tab */}
            {activeTab === 'fechada' && (
                <div className="flex items-center gap-2 bg-white p-1 rounded-xl border border-slate-200 shadow-sm overflow-x-auto w-full xl:w-auto">
                    <div className="px-3 py-2 text-xs font-bold text-slate-400 uppercase border-r border-slate-100 flex items-center gap-2">
                        <Filter size={14} /> Filtros
                    </div>
                    <select 
                        value={filterMonth} onChange={e => setFilterMonth(e.target.value)}
                        className="bg-transparent text-sm font-medium text-slate-600 outline-none cursor-pointer hover:bg-slate-50 rounded-lg px-2 py-1"
                    >
                  ...
                  <div key={comanda.id} className="group bg-white rounded-2xl p-6 border border-slate-100 hover:border-emerald-200 shadow-sm hover:shadow-lg transition-all flex flex-col relative overflow-hidden">
                      ...
                      <div className="pl-3 mb-4">
                          <div className="flex justify-between items-start mb-2">
                              <div>
                                  <h3 className="font-bold text-lg text-slate-800 leading-tight">{comanda.description}</h3>
                                  <div className="flex items-center gap-2 mt-1 text-sm text-slate-500">
                                      <User size={14} /> {comanda.patientName}
                                  </div>
                              </div>
                              <span className={`px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider ${
                                  comanda.type === 'pacote' ? 'bg-purple-50 text-purple-700' : 'bg-orange-50 text-orange-700'
                              }`}>
                                  {comanda.type}
                              </span>
                          </div>
                          <div className="flex items-center gap-4 text-xs text-slate-400 font-medium">
                              ...
                              <span className="flex items-center gap-1"><Layers size={12} /> {comanda.items.reduce((acc, i) => acc + i.quantity, 0)} itens</span>
                          </div>
                      </div>
                      ...
                      <div className="pl-3 mt-auto pt-4 border-t border-slate-50">
                          <div className="flex justify-between items-end mb-2">
                              <div>
                                  <div className="text-[10px] uppercase font-bold text-slate-400">Pago</div>
                                  <div className="text-sm font-bold text-emerald-600">{formatCurrency(comanda.paidValue)}</div>
                              </div>
                              <div className="text-right">
                                  <div className="text-[10px] uppercase font-bold text-slate-400">Total</div>
                                  <div className="text-lg font-display font-bold text-slate-800">{formatCurrency(comanda.totalValue)}</div>
                              </div>
                          </div>
                          <div className="w-full bg-slate-100 rounded-full h-1.5 mb-4 overflow-hidden">
                              <div className={`h-full rounded-full transition-all duration-1000 ${isFullyPaid ? 'bg-emerald-500' : 'bg-blue-500'}`} style={{ width: `${progress}%` }}></div>
                          </div>

                          <div className="flex gap-2">
                              <button onClick={() => handleOpenModal(comanda)} className="flex-1 py-2 rounded-lg bg-slate-50 text-slate-600 text-xs font-bold hover:bg-slate-100 hover:text-indigo-600 transition-colors flex items-center justify-center gap-2">
                                  <Edit3 size={14} /> Detalhes
                              </button>
                              <button onClick={() => handleDelete(comanda.id)} className="p-2 rounded-lg bg-red-50 text-red-500 hover:bg-red-100 transition-colors">
                                  <Trash2 size={16} />
                              </button>
                          </div>
                      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-[fadeIn_0.2s_ease-out]">
            <div className="bg-white w-full max-w-3xl rounded-[24px] shadow-2xl animate-[slideUpFade_0.3s_ease-out] overflow-hidden flex flex-col max-h-[90vh]">
                <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/80">
                    <div>
                        <h3 className="text-xl font-display font-bold text-slate-800">{editingComanda.id ? 'Editar Comanda' : 'Nova Comanda'}</h3>
                        <p className="text-xs text-slate-500 mt-1">Configure os serviços, valores e forma de pagamento.</p>
                    </div>
                ...
                <div className="p-8 space-y-6 overflow-y-auto custom-scrollbar">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="md:col-span-2">
                             ...
                             <input 
                                type="text" 
                                className="w-full p-3 rounded-xl border border-slate-200 outline-none font-medium text-slate-700 focus:ring-2 focus:ring-emerald-100" 
                                placeholder="Ex: Tratamento de Ansiedade - Outubro"
                        ...
                        <div>
                             ...
                             <div className="relative">
                                <select 
                                    className="w-full p-3 rounded-xl border border-slate-200 bg-white outline-none font-medium text-slate-700 appearance-none focus:ring-2 focus:ring-emerald-100"
                        ...
                        <div>
                            ...
                            <div className="flex bg-slate-100 p-1 rounded-xl">
                                <button 
                                    onClick={() => setEditingComanda({...editingComanda, type: 'servico', items: []})}
                                    className={`flex-1 py-2 text-sm font-bold rounded-lg transition-all ${editingComanda.type === 'servico' ? 'bg-white text-indigo-700 shadow-sm' : 'text-slate-500'}`}
                                >
                                    Serviço
                                </button>
                                <button 
                                    onClick={() => setEditingComanda({...editingComanda, type: 'pacote', items: []})}
                                    className={`flex-1 py-2 text-sm font-bold rounded-lg transition-all ${editingComanda.type === 'pacote' ? 'bg-white text-purple-700 shadow-sm' : 'text-slate-500'}`}
                                >
                                    Pacote
                                </button>
                            </div>
                    ...
                    <div className="bg-slate-50 p-6 rounded-2xl border border-slate-100">
                        <h4 className="font-bold text-slate-700 mb-4 flex items-center gap-2">
                            ...
                            {editingComanda.type === 'servico' ? 'Adicionar Serviços' : 'Selecionar Pacote'}
                        </h4>
                        ...
                        {editingComanda.type === 'servico' ? (
                            <div className="space-y-4">
                                <div className="flex gap-2">
                                    <div className="flex-1 relative">
                                        <select 
                                            id="service-select"
                                            className="w-full p-3 rounded-xl border border-slate-200 bg-white outline-none text-sm"
                                        >
                                ...
                                {/* Frequency Settings for Services */}
                                <div className="grid grid-cols-2 gap-4 pt-2">
                                     <div>
                                        <label className="text-xs font-bold text-slate-400 uppercase">Frequência</label>
                                        <select 
                                            value={editingComanda.frequency || 'unica'}
                                     ...
                                     <div>
                                        <label className="text-xs font-bold text-slate-400 uppercase">Dia da Recorrência</label>
                                        <select 
                                            value={editingComanda.recurrenceDay || ''}
                                            onChange={e => setEditingComanda({...editingComanda, recurrenceDay: e.target.value})}
                                            className="w-full mt-1 p-2 bg-white border border-slate-200 rounded-lg text-sm"
                                            disabled={editingComanda.frequency === 'unica'}
                                        >
                            ...
                            <div>
                                ...
                                >
                                    <option value="">Selecione um pacote pré-definido...</option>
                                    {MOCK_PACKAGES.map(p => (
                                        <option key={p.id} value={p.id}>{p.name} - {formatCurrency(p.totalPrice)}</option>
                                    ))}
                                </select>
                            </div>
                        )}
                        ...
                        <div className="mt-6 space-y-2">
                            {editingComanda.items?.map((item, idx) => (
                                <div key={idx} className="flex items-center justify-between bg-white p-3 rounded-xl border border-slate-200 shadow-sm">
                                    <div className="flex-1">
                                        <div className="text-sm font-bold text-slate-700">{item.serviceName}</div>
                                    ...
                                    <div className="flex items-center gap-4">
                                        ...
                                        <button 
                                            onClick={() => {
                                                const newItems = editingComanda.items?.filter((_, i) => i !== idx);
                            ...
                            {(!editingComanda.items || editingComanda.items.length === 0) && (
                                <div className="text-center text-slate-400 text-sm py-2">Nenhum item adicionado.</div>
                            )}
                        </div>
                    </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                 ...
                                 <div>
                                     ...
                                     <div className="flex gap-2">
                                        <select 
                                            ...
                                            onChange={e => setEditingComanda({...editingComanda, discountType: e.target.value as any})}
                                        ...
                                        <input 
                                            ...
                                            onChange={e => setEditingComanda({...editingComanda, discountValue: parseFloat(e.target.value)})}
                                            className="flex-1 p-3 border border-slate-200 rounded-xl outline-none"
                                        />
                                     </div>
                                 </div>
                                 <div>
                                     <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Valor Pago</label>
                                     <div className="relative">
                                         <DollarSign className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={16} />
                                         <input 
                                            type="number" 
                                            value={editingComanda.paidValue}
                                            onChange={e => setEditingComanda({...editingComanda, paidValue: parseFloat(e.target.value)})}
                                            className="w-full pl-10 pr-4 p-3 border border-slate-200 rounded-xl outline-none font-bold text-emerald-600"
                                        />
                                     </div>
                                 </div>
                             </div>
                             ...
                             <div className="bg-slate-50 rounded-2xl p-6 flex flex-col justify-center space-y-2">
                                 ...
                                 <div className="flex justify-between text-sm font-bold text-emerald-600 pt-1">
                                     <span>Restante</span>
                                     <span>{formatCurrency(Math.max(0, (editingComanda.totalValue || 0) - (editingComanda.paidValue || 0)))}</span>
                                 </div>
                             </div>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2 mt-4">
                        <label className="flex items-center gap-2 cursor-pointer select-none">
                            <input 
                                type="checkbox" 
                                checked={editingComanda.status === 'fechada'}
                                onChange={e => setEditingComanda({...editingComanda, status: e.target.checked ? 'fechada' : 'aberta'})}
                                className="w-5 h-5 rounded border-slate-300 text-emerald-600 focus:ring-emerald-500"
                            />
                            <span className="font-bold text-slate-700">Marcar como Fechada/Finalizada</span>
                        </label>
                    </div>
                ...
                <div className="p-6 border-t border-slate-100 bg-slate-50 flex justify-end gap-3">
                    ...
                    <button onClick={handleSaveComanda} className="px-8 py-3 rounded-xl font-bold bg-emerald-600 text-white hover:bg-emerald-700 shadow-lg flex items-center gap-2">
                        <CheckCircle size={18} /> Salvar Comanda
                    </button>
                </div>
            </div>
        </div>
