...
export const Services: React.FC = () => {
  ...
  return (
    <div className="space-y-6 animate-fadeIn font-sans pb-24">
      ...
      {/* FILTERS & TABS BAR */}
      <div className="bg-white p-4 rounded-3xl border border-slate-100 shadow-sm flex flex-col lg:flex-row gap-4 justify-between items-center sticky top-4 z-40 backdrop-blur-md bg-white/90">
              ...
              <div className="bg-white w-full max-w-lg rounded-[3rem] shadow-2xl overflow-hidden flex flex-col border border-white/20">
                  <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/30">
                      <div>
                          ...
                          <p className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.2em]">CONFIGURAÇÕES NA AGENDA</p>
                      </div>
                      <button onClick={() => setIsServiceModalOpen(false)} className="p-3 hover:bg-white hover:shadow-md rounded-2xl text-slate-400 ring-1 ring-slate-200 transition-all"><X size={18}/></button>
                  </div>
                  
                  <div className="p-10 space-y-6 overflow-y-auto max-h-[70vh] custom-scrollbar">
                      <div>
                          <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2.5 px-1">Nome do Atendimento</label>
                          <input 
                              type="text" 
                              className="w-full text-sm font-black p-4 rounded-[1.5rem] border-2 border-slate-100 bg-slate-50 outline-none focus:bg-white focus:border-indigo-400 focus:ring-8 focus:ring-indigo-100/30 transition-all" 
                      ...
                      <div className="grid grid-cols-2 gap-5">
                          <div>
                              ...
                              <input 
                                  type="text" 
                                  className="w-full text-sm font-black p-4 rounded-[1.5rem] border-2 border-slate-100 bg-slate-50 outline-none focus:bg-white focus:border-indigo-400 transition-all" 
                          ...
                          <div>
                              ...
                              <input 
                                  type="number" 
                                  className="w-full text-sm font-black p-4 rounded-[1.5rem] border-2 border-slate-100 bg-slate-50 outline-none focus:bg-white focus:border-indigo-400 transition-all text-center" 
                      ...
                      <div className="grid grid-cols-2 gap-5">
                          <div>
                              <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2.5 px-1">Valor Venda</label>
                          ...
                          <div>
                              <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2.5 px-1">Custo Prof.</label>
                              <div className="relative">
                                  <DollarSign className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={18} />
                                  <input 
                                      type="number" 
                                      className="w-full text-lg font-black p-4 pl-11 rounded-[1.5rem] border-2 border-slate-100 bg-slate-50 outline-none focus:bg-white focus:border-indigo-400 transition-all text-slate-600" 
                      ...
                      <div>
                          <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2.5 px-1">Modalidade</label>
                          <div className="flex bg-slate-100 p-1.5 rounded-2xl">
                              <button onClick={() => setEditingService({...editingService, modality: 'presencial'})} className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${editingService.modality === 'presencial' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-indigo-400'}`}>Presencial</button>
                              <button onClick={() => setEditingService({...editingService, modality: 'online'})} className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${editingService.modality === 'online' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-indigo-400'}`}>Online</button>
                          </div>
                      </div>
                  </div>

                  <div className="p-8 border-t border-slate-50 bg-slate-50/20 flex justify-end gap-4 px-10 pb-10">
                      <button onClick={() => setIsServiceModalOpen(false)} className="px-6 py-3 text-xs font-black text-slate-400 hover:text-slate-600 transition-colors uppercase tracking-widest">DESCARTAR</button>
                      <button onClick={handleSaveService} className="px-10 py-3 text-xs font-black text-white bg-indigo-600 hover:bg-indigo-700 rounded-[1.5rem] shadow-xl shadow-indigo-200 transition-all flex items-center gap-2 uppercase tracking-widest transform active:scale-95">
      ...
      {isPackageModalOpen && editingPackage && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md animate-fadeIn">
              <div className="bg-white w-full max-w-2xl rounded-[3rem] shadow-2xl overflow-hidden flex flex-col border border-white/20">
                  <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/30">
                      <div>
                          <h3 className="text-xl font-black text-slate-800">Estruturar Pacote</h3>
                          <p className="text-[10px] font-black text-emerald-500 uppercase tracking-[0.2em]">COMBO DE ATENDIMENTOS</p>
                      </div>
                      <button onClick={() => setIsPackageModalOpen(false)} className="p-3 hover:bg-white hover:shadow-md rounded-2xl text-slate-400 ring-1 ring-slate-200 transition-all"><X size={18}/></button>
                  </div>
                  
                  <div className="p-10 space-y-8 overflow-y-auto max-h-[75vh] custom-scrollbar">
                      <div>
                          <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2.5 px-1">Título do Pacote</label>
                          <input 
                              type="text" 
                              className="w-full text-base font-black p-5 rounded-[2rem] border-2 border-slate-100 bg-slate-50 outline-none focus:bg-white focus:border-indigo-400 focus:ring-8 focus:ring-indigo-100/30 transition-all" 
                      ...
                      <div className="bg-indigo-50/40 p-8 rounded-[2.5rem] border border-indigo-100/50">
                          <div className="flex justify-between items-center mb-6">
                              <h4 className="text-[11px] font-black text-indigo-900 uppercase tracking-widest flex items-center gap-3 px-2">
                                  <Layers size={18}/> Serviços Selecionados
                              </h4>
                              <div className="relative group">
                                <button className="h-10 px-4 bg-white border border-indigo-200 rounded-2xl flex items-center gap-2 text-[10px] font-black text-indigo-600 hover:bg-indigo-600 hover:text-white transition-all shadow-sm uppercase tracking-widest">
                          ...
                          <div className="space-y-3">
                              {(!editingPackage.items || editingPackage.items.length === 0) ? (
                                  <div className="text-center py-10 border-2 border-dashed border-indigo-100 rounded-[2rem] bg-white/50">
                                      <p className="text-[11px] font-black text-indigo-300 uppercase tracking-widest">Nenhum serviço no pacote</p>
                                  </div>
                              ) : (
                                  editingPackage.items.map((item, idx) => {
                                      const s = services.find(srv => srv.id === item.serviceId);
                                      return (
                                          <div key={idx} className="flex gap-4 items-center animate-slideIn bg-white p-3 rounded-2xl border border-indigo-100 shadow-sm">
                                              <div className="flex-1 text-xs font-black text-slate-700 truncate">{s?.name}</div>
                                              <div className="flex items-center gap-4">
                                                  <div className="flex items-center gap-2 bg-slate-50 p-1.5 rounded-xl border border-slate-100">
                                                    <span className="text-[10px] font-black text-slate-300">QTD:</span>
                                                    <input type="number" min="1" className="w-10 bg-transparent border-0 font-black text-slate-700 text-xs text-center" value={item.quantity} onChange={(e) => {const newItems = [...(editingPackage.items || [])]; newItems[idx].quantity = parseInt(e.target.value) || 1; handlePackageItemChange(newItems);}} />
                                                  </div>
                                                  <span className="text-xs font-black text-indigo-400 tracking-tighter w-20 text-right">{s ? formatCurrency(s.price * item.quantity) : '-'}</span>
                                                  <button onClick={() => {const newItems = editingPackage.items!.filter((_, i) => i !== idx); handlePackageItemChange(newItems);}} className="p-2 border border-slate-100 rounded-xl text-slate-300 hover:text-red-500 hover:bg-red-50 transition-all"><X size={16}/></button>
                                              </div>
                                          </div>
                                      );
                                  })
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-end p-2">
                        <div className="space-y-4">
                            <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">Configurar Desconto</label>
                            <div className="flex bg-slate-100 p-1.5 rounded-[1.8rem] border border-slate-200">
                                <button onClick={() => handlePackageDiscountChange('percentage', editingPackage.discountValue || 0)} className={`flex-1 py-3 rounded-xl text-[10px] font-black transition-all flex items-center justify-center gap-2 ${editingPackage.discountType === 'percentage' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-400'}`}><Percent size={14}/> %</button>
                                <button onClick={() => handlePackageDiscountChange('fixed', editingPackage.discountValue || 0)} className={`flex-1 py-3 rounded-xl text-[10px] font-black transition-all flex items-center justify-center gap-2 ${editingPackage.discountType === 'fixed' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-400'}`}><DollarSign size={14}/> VALOR</button>
                                <input type="number" className="w-20 bg-transparent border-0 font-black text-indigo-700 text-center text-lg outline-none" value={editingPackage.discountValue || 0} onChange={(e) => handlePackageDiscountChange(editingPackage.discountType || 'percentage', parseFloat(e.target.value) || 0)} />
                            </div>
                        </div>
                        <div className="bg-slate-900 rounded-[2rem] p-6 text-white text-right">
                            <p className="text-[10px] font-black text-indigo-300 uppercase tracking-widest mb-1">Preço Final do Combo</p>
                            <p className="text-3xl font-black text-white tabular-nums">{formatCurrency(editingPackage.totalPrice || 0)}</p>
                        </div>
                      </div>
                  </div>

                  <div className="p-8 border-t border-slate-50 bg-slate-50/20 flex justify-end gap-4 px-12 pb-12">
                      <button onClick={() => setIsPackageModalOpen(false)} className="px-8 py-4 text-xs font-black text-slate-400 hover:text-slate-600 uppercase tracking-widest">DESCARTAR</button>
                      <button onClick={handleSavePackage} className="px-14 py-4 text-xs font-black text-white bg-indigo-600 hover:bg-slate-800 rounded-[1.8rem] shadow-2xl transition-all flex items-center gap-3 uppercase tracking-widest active:scale-95">
