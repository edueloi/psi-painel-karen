...
import {
  ...
  Copy, CheckCircle, Calendar, BarChart2, AlertCircle, Info,
...
interface Patient { id: string; name?: string; full_name?: string; status?: string; }

// ─── Constants ───────────────────────────────────────────────────────────────
const BLOCK_CONFIG = {
  D: { label: 'Dominância',   color: '#dc2626', bg: '#fef2f2', border: '#fecaca', icon: <Zap size={14} />,    desc: 'Direto, assertivo, orientado a resultado e ação' },
  I: { label: 'Influência',   color: '#d97706', bg: '#fffbeb', border: '#fde68a', icon: <Star size={14} />,   desc: 'Comunicativo, caloroso, persuasivo e social' },
  S: { label: 'Estabilidade', color: '#16a34a', bg: '#f0fdf4', border: '#bbf7d0', icon: <Shield size={14} />, desc: 'Estável, acolhedor, paciente, resistente a mudanças' },
  C: { label: 'Conformidade', color: '#2563eb', bg: '#eff6ff', border: '#bfdbfe', icon: <Target size={14} />, desc: 'Analítico, cuidadoso, organizado, crítico' },
} as const;

const FACTOR_DETAILS: Record<string, { beliefs: string[]; attention: string[]; tcc: string[] }> = {
  D: { tcc: ['Impaciência', 'Necessidade de controle', 'Baixa tolerância à frustração'], beliefs: ['"Preciso resolver isso agora."', '"Se eu não assumir, nada vai funcionar."', '"Demonstrar fraqueza é perigoso."'], attention: ['Irritabilidade', 'Impulsividade', 'Autocobrança por desempenho', 'Conflito interpessoal'] },
  I: { tcc: ['Busca de validação', 'Necessidade de aprovação', 'Sensibilidade à rejeição'], beliefs: ['"Preciso ser aceito(a)."', '"Se não gostarem de mim, há algo errado comigo."', '"Preciso manter o clima bom."'], attention: ['Dependência de validação externa', 'Dificuldade em colocar limites', 'Oscilação emocional'] },
  S: { tcc: ['Evitação de conflito', 'Medo de ruptura', 'Dificuldade de mudança'], beliefs: ['"É melhor evitar problema."', '"Mudanças podem dar errado."', '"Preciso manter tudo em paz."'], attention: ['Passividade', 'Medo de desagradar', 'Permanência em contextos ruins'] },
  C: { tcc: ['Perfeccionismo', 'Excesso de análise', 'Medo de errar', 'Procrastinação'], beliefs: ['"Não posso errar."', '"Preciso ter certeza antes."', '"Só posso agir quando estiver perfeito."'], attention: ['Ansiedade', 'Ruminação', 'Padrões inflexíveis', 'Autocrítica elevada'] },
};
...
function DiscRadarChart({ d, i, s, c }: { d: number; i: number; s: number; c: number }) {
  ...
  const axes = [
    { key: 'D', angle: -90, val: d, color: '#dc2626' },
    { key: 'I', angle: 0,   val: i, color: '#d97706' },
    { key: 'S', angle: 90,  val: s, color: '#16a34a' },
    { key: 'C', angle: 180, val: c, color: '#2563eb' },
  ];
    ...
    <svg viewBox={`0 0 ${size} ${size}`} width="100%" style={{ maxWidth: 220 }}>
      ...
      {levels.map(lv => (
        <polygon
          ...
          fill="none" stroke={lv === 5 ? '#cbd5e1' : '#e2e8f0'} strokeWidth={lv === 5 ? 1.5 : 1}
        />
      ))}
      {/* Level labels on D axis */}
      {levels.map(lv => {
        const [x, y] = toXY(-90, (lv / 5) * maxR);
        return <text key={lv} x={x + 4} y={y} fontSize="7" fill="#94a3b8">{lv}</text>;
      })}
      {/* Axis lines */}
      {axes.map(a => {
        const [x, y] = toXY(a.angle, maxR);
        return <line key={a.key} x1={cx_} y1={cy_} x2={x} y2={y} stroke="#e2e8f0" strokeWidth={1.5} strokeDasharray="3,2" />;
      })}
      {/* Data polygon fill */}
      <polygon points={polygon} fill="rgba(37,99,235,0.12)" stroke="#2563eb" strokeWidth={2.5} strokeLinejoin="round" />
      {/* Data points */}
      {axes.map(a => {
        const r = (a.val / 5) * maxR;
        const [x, y] = toXY(a.angle, r);
        return <circle key={a.key} cx={x} cy={y} r={5} fill={a.color} stroke="#fff" strokeWidth={2} />;
      })}
      {/* Labels */}
      {axes.map(a => {
        const [lx, ly] = toXY(a.angle, maxR + 18);
        const [vx, vy] = toXY(a.angle, maxR + 30);
        return (
          <g key={a.key}>
            <text x={lx} y={ly} textAnchor="middle" dominantBaseline="central" fontSize={11} fontWeight={900} fill={a.color}>{a.key}</text>
            <text x={vx} y={vy} textAnchor="middle" dominantBaseline="central" fontSize={9} fill="#64748b">{a.val.toFixed(2)}</text>
          </g>
        );
      })}
    </svg>
...
function DiscBarChart({ d, i, s, c }: { d: number; i: number; s: number; c: number }) {
  const scores = [
    { key: 'D', label: 'Dominância',   val: d, ...BLOCK_CONFIG.D },
    { key: 'I', label: 'Influência',   val: i, ...BLOCK_CONFIG.I },
    { key: 'S', label: 'Estabilidade', val: s, ...BLOCK_CONFIG.S },
    { key: 'C', label: 'Conformidade', val: c, ...BLOCK_CONFIG.C },
  ];
  return (
            ...
            <div className="flex items-center justify-between mb-1">
              <div className="flex items-center gap-2">
                <span style={{ background: s_.bg, color: s_.color, borderColor: s_.border }}
                  className="inline-flex items-center gap-1 px-2 py-0.5 rounded-lg border text-[10px] font-black uppercase tracking-wide">
                  {s_.key} — {s_.label}
                </span>
              </div>
              <div className="flex items-center gap-2">
                <span className="text-[10px] font-bold" style={{ color: lv.color }}>{lv.label}</span>
                <span className="text-base font-black" style={{ color: s_.color }}>{s_.val.toFixed(2)}</span>
              </div>
            </div>
            <div className="h-3 bg-slate-100 rounded-full overflow-hidden">
              <div
                className="h-3 rounded-full transition-all duration-700"
                style={{ width: `${pct}%`, background: s_.color, minWidth: pct > 0 ? 8 : 0 }}
              />
            </div>
            <p className="text-[10px] text-slate-400 mt-0.5">{s_.desc}</p>
          </div>
        );
      })}
    </div>
  ...
  return (
    <div className="flex gap-1.5 items-end" style={{ height: 28 }}>
      {(['D', 'I', 'S', 'C'] as const).map(k => {
        ...
        const h = Math.max(4, Math.round(((val - 1) / 4) * 24));
        return (
          <div key={k} className="flex flex-col items-center gap-0.5">
            <div style={{ height: h, width: 8, background: BLOCK_CONFIG[k].color, borderRadius: 3 }} />
...
export default function Disc() {
  ...
  const generateAurora = async (result: DiscResult) => {
    ...
    try {
      ...
      const resp = await api.post<any>('/ai/analyze-form', {
        ...
        interpretations: [
          { resultTitle: 'Dominância', description: `Média D: ${scores.D.toFixed(2)}` },
          { resultTitle: 'Influência', description: `Média I: ${scores.I.toFixed(2)}` },
          { resultTitle: 'Estabilidade', description: `Média S: ${scores.S.toFixed(2)}` },
          { resultTitle: 'Conformidade', description: `Média C: ${scores.C.toFixed(2)}` },
        ],
  ...
  return (
    <div className="p-6 space-y-6 max-w-5xl mx-auto">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-slate-400 font-medium mt-0.5">Análise Comportamental Adaptada para TCC</p>
          </div>
        </div>
        <div className="flex gap-2 flex-wrap">
          <Button variant="ghost" size="sm" radius="xl" leftIcon={<Info size={15} />} onClick={() => setManualOpen(true)}>
            Manual de Uso
          </Button>
      ...
      {/* Filter */}
      <div className="bg-white border border-slate-100 rounded-3xl p-4 shadow-sm flex flex-col sm:flex-row gap-3 items-center">
      ...
      {!loading && results.length === 0 && (
        <div className="bg-gradient-to-br from-indigo-50 to-violet-50 border border-indigo-100 rounded-3xl p-8 text-center">
          ...
          <h3 className="font-black text-slate-700 mb-1">Nenhuma avaliação DISC encontrada</h3>
          <p className="text-sm text-slate-500 mb-4 max-w-md mx-auto">
            Compartilhe o link do DISC com seus pacientes. Quando responderem, os resultados aparecerão aqui com análise completa por fator D/I/S/C.
          </p>
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {results.map(result => {
            ...
            return (
              <div key={result.id}
                className={`bg-white border rounded-3xl shadow-sm overflow-hidden transition-all ${isExpanded ? 'border-indigo-200 shadow-indigo-100' : 'border-slate-100'}`}>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      <p className="font-black text-slate-900 text-sm truncate">{result.patient_name}</p>
                    ...
                    <p className="text-xs text-slate-400 mt-0.5">{new Date(result.created_at).toLocaleDateString('pt-BR', { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' })}</p>
                  </div>
                  ...
                  <div className="hidden sm:flex gap-3 shrink-0">
                    {(['D','I','S','C'] as const).map(k => (
                      <div key={k} className="text-center">
                        <p className="text-base font-black" style={{ color: BLOCK_CONFIG[k].color }}>
                          {(result[`score_${k.toLowerCase()}` as keyof DiscResult] as number).toFixed(1)}
                        </p>
                  ...
                  <div className="border-t border-slate-100 p-6 space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div className="bg-slate-50 rounded-2xl p-4 flex flex-col items-center">
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Gráfico Radar</p>
                        <DiscRadarChart
                          d={result.score_d} i={result.score_i}
                          s={result.score_s} c={result.score_c}
                        />
                      </div>
                      ...
                      <div className="bg-slate-50 rounded-2xl p-4">
                        ...
                        <div className="mt-4 pt-4 border-t border-slate-200 flex items-center gap-2 flex-wrap">
                          ...
                          {[['1.0–2.4','Baixo','#94a3b8'],['2.5–3.4','Moderado','#d97706'],['3.5–5.0','Forte','#16a34a']].map(([r,l,c]) => (
                            <span key={l} className="text-[9px] font-bold px-2 py-0.5 rounded-full" style={{ background: c + '18', color: c }}>{r} — {l}</span>
                          ))}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {/* Dominant factor details */}
                      <div style={{ background: BLOCK_CONFIG[dominant.key].bg, borderColor: BLOCK_CONFIG[dominant.key].border }}
                        className="rounded-2xl border p-4">
                        <p className="text-[10px] font-black uppercase tracking-widest mb-2" style={{ color: BLOCK_CONFIG[dominant.key].color }}>
                          Fator Dominante — {dominant.key} ({BLOCK_CONFIG[dominant.key].label})
                        </p>
                        <p className="text-xs text-slate-600 mb-3">Na TCC, pode aparecer como:</p>
                        <div className="space-y-1 mb-3">
                          {FACTOR_DETAILS[dominant.key].tcc.map(t => (
                            <span key={t} className="inline-flex items-center gap-1 mr-1 mb-1 text-[10px] font-bold px-2 py-0.5 rounded-full bg-white border text-slate-600">{t}</span>
                          ))}
                        </div>
                        <p className="text-[10px] font-black text-slate-500 uppercase tracking-wide mb-1">Crenças comuns</p>
                        {FACTOR_DETAILS[dominant.key].beliefs.map(b => (
                          <p key={b} className="text-xs text-slate-500 italic mb-0.5">{b}</p>
                        ))}
                      </div>
                      ...
                      <div className="rounded-2xl border border-indigo-100 bg-indigo-50 p-4">
                        ...
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-wide mb-2">Pontos de atenção clínicos</p>
                        <div className="flex flex-wrap gap-1">
                          {FACTOR_DETAILS[dominant.key].attention.slice(0, 3).map(a => (
                            <span key={a} className="text-[9px] font-bold px-2 py-0.5 rounded-full bg-white border border-indigo-100 text-indigo-600">{a}</span>
                          ))}
                          {FACTOR_DETAILS[second.key].attention.slice(0, 2).map(a => (
                            <span key={a} className="text-[9px] font-bold px-2 py-0.5 rounded-full bg-white border border-slate-100 text-slate-500">{a}</span>
                          ))}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-gradient-to-br from-violet-50 to-indigo-50 border border-violet-100 rounded-2xl p-5">
                      <div className="flex items-center justify-between mb-3">
                        ...
                        {!aurora && (
                          <Button
                            size="xs" variant="primary" radius="xl"
                        ...
                        {aurora && (
                          <Button size="xs" variant="ghost" radius="xl" leftIcon={<Sparkles size={12} />}
                            isLoading={analyzingId === result.id}
                            loadingText="Analisando..."
                            onClick={() => generateAurora(result)}>
                            Regenerar
                      ...
                      {aurora ? (
                        <div
                          ...
                          dangerouslySetInnerHTML={{
                            __html: aurora
                              .replace(/^:\s+/gm, '')
                              .replace(/\*\*(.*?)\*\*/g, '<strong style="display:block;margin-top:20px;margin-bottom:8px;color:#1e293b;font-size:12px;font-weight:900;border-left:3px solid #7c3aed;padding-left:10px;text-transform:uppercase;">$1</strong>')
                              .replace(/\n\n/g, '<br/><br/>').replace(/\n/g, '<br/>')
                          }}
                        />
                      ) : (
                        <p className="text-xs text-slate-400 italic">
                          Clique em "Gerar Análise" para que a Aurora crie uma análise clínica detalhada baseada no perfil DISC deste paciente.
                        </p>
                      )}
                    </div>
                    ...
                    <details className="group">
                      <summary className="flex items-center justify-between cursor-pointer py-3 px-4 bg-slate-50 rounded-2xl hover:bg-slate-100 transition-colors list-none">
                        <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Ver Respostas Completas</span>
                      ...
                      <div className="mt-3 space-y-1 max-h-64 overflow-y-auto">
                        {Object.entries(result.answers).map(([qId, answer]) => (
                          <div key={qId} className="flex items-center justify-between px-3 py-2 rounded-xl hover:bg-slate-50">
                            <span className="text-xs text-slate-500 font-medium">{qId.toUpperCase()}</span>
                            <span className="text-xs font-bold text-slate-800">{String(answer)}</span>
                          </div>
                        ))}
                      </div>
                    </details>
      ...
      <Modal
        ...
        title="Manual de Uso — DISC para TCC"
        subtitle="Como interpretar e usar os resultados na sessão"
        maxWidth="2xl"
        footer={<Button variant="ghost" radius="xl" onClick={() => setManualOpen(false)}>Fechar</Button>}
      >
        <div className="space-y-5 text-sm text-slate-700 max-h-[60vh] overflow-y-auto pr-2">
          {/* Objetivo */}
          <div className="bg-indigo-50 border border-indigo-100 rounded-2xl p-4">
            <p className="text-[10px] font-black text-indigo-600 uppercase tracking-widest mb-2">Objetivo</p>
            <p className="text-sm leading-relaxed">Identificar tendências de comportamento, tomada de decisão, relação com pessoas, ritmo de ação e organização, para ajudar na compreensão de padrões cognitivos e comportamentais na TCC.</p>
          </div>

          {/* Escala */}
          <div>
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Escala de Respostas</p>
            <div className="flex gap-2 flex-wrap">
          ...
          {/* Cálculo */}
          <div>
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Como Calcular</p>
            <div className="grid grid-cols-2 gap-2">
              {[['D','Dominância','÷ 8','q1–q8'],['I','Influência','÷ 7','q9–q15'],['S','Estabilidade','÷ 7','q16–q22'],['C','Conformidade','÷ 8','q23–q30']].map(([k,l,d,q]) => (
                <div key={k} style={{ background: BLOCK_CONFIG[k as keyof typeof BLOCK_CONFIG].bg, borderColor: BLOCK_CONFIG[k as keyof typeof BLOCK_CONFIG].border }}
                  ...
                  <p className="text-xs text-slate-500">{q} {d}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* Faixas */}
          <div>
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Faixas de Interpretação</p>
            <div className="space-y-2">
              {[['1.0 – 2.4','Traço pouco presente','#94a3b8'],['2.5 – 3.4','Traço moderado','#d97706'],['3.5 – 5.0','Traço forte / predominante','#16a34a']].map(([r,l,c]) => (
                <div key={l} className="flex items-center gap-3">
                  <span className="font-mono text-xs font-bold w-20 shrink-0" style={{ color: c }}>{r}</span>
          ...
          <div className="bg-emerald-50 border border-emerald-100 rounded-2xl p-4">
            <p className="text-[10px] font-black text-emerald-600 uppercase tracking-widest mb-2">Modelo de Devolutiva</p>
          ...
          <div className="bg-amber-50 border border-amber-100 rounded-2xl p-4">
            ...
            <p className="text-xs text-slate-600 leading-relaxed">Apresentar como <strong>"instrumento de apoio para autoconhecimento e compreensão de padrões comportamentais"</strong>, e não como teste psicológico formal ou diagnóstico de personalidade.</p>
          </div>
        </div>
      </Modal>
    </div>
