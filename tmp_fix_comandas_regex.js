...
// Use RegExp with \s* to match regardless of space/tab indentation
function flexibleReplace(oldText, newText) {
    // Escape special regex chars
    const escaped = oldText.replace(/[.*+?^${}()|[\]\\]/g, '\\$&').replace(/\s+/g, '\\s+');
...
const descAndSessionsOld = `<td className="px-8 py-5">
...
const descAndSessionsNew = `<td className="px-8 py-5">
                                   <div className="max-w-[200px] truncate text-xs font-bold text-slate-500/70">{c.description || 'Consulta/Serviço'}</div>
                               </td>
                               <td className="px-8 py-5">
...
flexibleReplace(tableRowOld, tableRowNew);
flexibleReplace(descAndSessionsOld, descAndSessionsNew);
...
flexibleReplace(onChangeOld, onChangeNew);
